...
const R2_BUCKET_NAME = process.env.R2_IMAGES_BUCKET_NAME || process.env.R2_BUCKET_NAME;

if (!R2_ENDPOINT || !R2_ACCESS_KEY_ID || !R2_SECRET_ACCESS_KEY || !R2_BUCKET_NAME) {
  console.error("❌ Missing R2 credentials.");
  console.error("Required: R2_ENDPOINT, R2_ACCESS_KEY_ID (or R2_TOKEN), R2_SECRET_ACCESS_KEY, R2_IMAGES_BUCKET_NAME (or R2_BUCKET_NAME)");
  process.exit(1);
}
...
// Team/board photos live in public/. Object keys mirror the runtime paths
// that getImageUrl() builds (e.g. "/board/2025/manan-shah.webp" -> key
// "board/2025/manan-shah.webp"), so the bucket is served at
// PUBLIC_IMAGE_CDN_URL with no path rewriting.
const PUBLIC_DIR = join(__dirname, "..", "public");
const IMAGE_DIRS = ["board", "team"];
...
async function main() {
  console.log("🚀 Uploading team/board images to R2...");
